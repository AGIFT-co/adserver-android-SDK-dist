...
バージョンは当社が案内するものを**完全一致で固定**してください（現在の案内バージョン: `0.0.2`）。当社は版ごとに動作確認したバージョンを案内します。
...
- **ファイルサイズは 500KB 以下を推奨**します。SDK は取得処理全体（接続〜ダウンロード完了）に **30 秒の上限**を設けており、低速回線では大きすぎるクリエイティブが時間内に完了せず altText 表示（VIEW 未計測）に落ちます（例: 5MB は持続 約 1.3Mbps 未満の回線で間に合いません）。自動リフレッシュ（既定 60 秒）の周期内で確実に表示するためにも、枠サイズに合わせた軽量な画像（数十〜数百 KB）で入稿してください。
