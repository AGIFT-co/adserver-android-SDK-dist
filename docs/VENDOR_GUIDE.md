...
| `jp.co.agift.adserver:sdk-admob`（アダプタ） | コア + AdMob カスタムイベント | あり（`play-services-ads`） |

- **直接配信だけ使う**なら、コア（`sdk`）のみ依存すれば `play-services-ads` を引き込みません（AD_ID を要求しない）。
- **AdMob メディエーション経由でも使う**なら、`sdk-admob` を依存します（コアも自動で入ります）。
...
`Application` または起動 `Activity` の `onCreate` で初期化します。

```kotlin
override fun onCreate() {
    super.onCreate()
    AdServerSDK.configure() // 組み込みの既定エンドポイントを使う
    // 任意: エンドポイントを明示指定する場合
    // AdServerSDK.configure(adServerBaseURL = "https://.../delivery", logBaseURL = "https://.../log")
}
```

> エンドポイント URL は SDK に組み込み済みです（確定後のバージョンを配布します）。
...
Jetpack Compose では `AndroidView { AdServerBannerView(...) }` でラップして使えます（SampleApp の `DirectTestScreen` を参照）。
...
- no-fill / エラーでも**枠は畳まれません**。`fallbackView` を設定すると、広告が無い間その View を枠内に表示します。
...
- [ ] `AdServerSDK.configure()` を `Application.onCreate()` で呼び出している
...
A: 新規は直接配信（`AdServerBannerView`）を推奨します。AdMob ウォーターフォールに組み込みたい既存ベンダーは AdMob 経路（付録 A）を使います。両者は共存できます。

**Q: 広告が表示されない**
A: `AdServerSDK.configure()` の呼び出しと、`adUnitId` が正しい UUID か確認してください。no-fill（在庫なし）の場合は `onAdFailedToLoad` に `AdServerError.NoFill` が渡ります。
